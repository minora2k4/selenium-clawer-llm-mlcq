...
const DATA_URL = '../output/questions_vi.json';
...
/**
 * Dữ liệu gốc (questions_vi.json) là một MẢNG các chủ đề:
 * [ { title: "Tên chủ đề", html: [ { question, options, answer, explanation }, ... ] }, ... ]
 *
 * Hàm này chuyển mảng thô đó thành mảng chủ đề đã chuẩn hoá:
...
function normalizeData(raw){
  if(!Array.isArray(raw)){
    throw new Error('Định dạng dữ liệu không hợp lệ: cần một mảng các chủ đề.');
  }

  let qid = 0;
  const topics = raw.map((topic, tIndex) => {
    const qlist = topic.html || [];
    ...
    return { index: tIndex, title: (topic.title || '').trim(), questions };
  });
