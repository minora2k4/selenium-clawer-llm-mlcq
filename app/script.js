(function(){
  ...
  /* ---------------- Markdown + công thức toán (KaTeX) ---------------- */
  // Tải marked.js (render Markdown) và KaTeX (render $...$ / $$...$$) từ CDN,
  // vì dữ liệu câu hỏi/giải thích có chứa cú pháp Markdown và LaTeX thô (vd: $t_{0}$).
  function loadCss(href){
    return new Promise((resolve, reject)=>{
      ...
      link.href = href;
      link.onload = () => resolve();
      link.onerror = () => reject(new Error('Không tải được CSS: '+href));
  ...
  function ensureLibs(){
    ...
    _libsReady = Promise.all([
      ...
      console.warn('Không tải được thư viện Markdown/KaTeX (có thể do mất mạng). Sẽ hiển thị văn bản thô.', err);
    });
    return _libsReady;
  }
  
  // Render một đoạn văn bản có thể chứa Markdown. inline=true -> không bọc trong <p>.
  function renderMD(text, inline){
    text = text || '';
    if(typeof marked === 'undefined'){
  ...
  // Quét một vùng DOM và render các công thức $...$, $$...$$, \(...\), \[...\] bằng KaTeX.
  function typesetMath(container){
    if(typeof renderMathInElement !== 'function') return;
    try{
      renderMathInElement(container || app, {
        delimiters: [
    ...
    }catch(e){ /* bỏ qua lỗi công thức lẻ */ }
  }
  ...
  function renderQuiz(){
    ...
    const optionsHtml = q.options.map(o=>`
      <div class="option ${selected===o.key?'selected':''}" data-key="${o.key}">
        ...
        <div class="opt-text">${renderMD(o.text, true)}</div>
      </div>`).join('');
    ...
    app.innerHTML = `
      ...
      <div class="q-card">
        <div class="q-text">${renderMD(q.question, false)}</div>
  ...
  function renderResult(){
    ...
    const reviewHtml = filtered.map((q,i)=>{
      ...
      return `<div class="review-card">
        ...
        <div class="rc-explain"><b>Giải thích:</b> ${renderMD(q.explanation || 'Không có giải thích.', false)}</div>
      </div>`;
  ...
  /* ---------------- Init ---------------- */
  Promise.all([loadData(), ensureLibs()])
    .then(([topics]) => {
      TOPICS = topics;
